...
====
**UI Method:**
1. Navigate to **Workloads** -> **Pods** in the OpenShift console. Filter by the `aap-rh1` namespace.
...
[.instruction]
====
**CLI Method:** Describe the failing Hub pod and PVC to find the specific scheduling or binding error.
====

[source,bash]
----
oc describe pod <failing-hub-pod> -n aap-rh1
oc describe pvc data-automation-hub -n aap-rh1
----
...
2. Scroll to the **Events** section to read the scheduler's error message (e.g., a failure to bind due to an access mode conflict).
====
...
* The failure is related to how the **Automation Hub** attempts to utilize storage. Review the pod events (`oc describe pod <failing-hub-pod>`) to see if there are errors related to binding a **Persistent Volume Claim (PVC)** or a **volume access mode** conflict. You need a storage class that supports **ReadWriteMany (RWX)**.